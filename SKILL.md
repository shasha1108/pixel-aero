...
> **启动时阅读 `references/methods.md`**（架构/形状/运动/材质/交互/色彩/音效 7 大通用原则，一次读完覆盖所有场景）。
...
```
用户描述 → 场景类型(封闭/开放) → 主物体拆解 → 副物体拆解 → 背景元素
...
基于拆解结果做 5 个决策：
- **画布类型**：固定画布（有玻璃容器）或全视口（开放场景）
- **运动模式**：FSM（自主生命体）或 React-only（植物/摆件）
- **玻璃类型**：穹顶（`h=w/2`）、球体（`border-radius:50%`）、或无玻璃
- **粒子类型**：Perlin 漩涡（孢子/萤火虫）、下落（雨/雪）、或无
- **音效**：drone 底噪（有/无）+ 交互 chime（至少一种）
